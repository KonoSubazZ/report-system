...
public class ComplexMutationServiceImpl implements ComplexMutationService {
	...
	@Override
	public List<Map> matchComplexMutation(String user,Integer report_id, Map<String,Object> result, Integer lang) {
		...
		//得到所有突变的Mutlist
		List<Map> thisGeneticmarkerVwList = analysisReportDao.getThisGeneticmarkerVwList(report_id);
		...
		int totalMutNum = 0; // 简单位点总数（CR + Somatic）
		...
		for (Map map : drug_var_list) {
			...
			try {
				reportCrService.handleDrugList(user, diseaseId, map, diseaseIdList, parentdiseaseIdList, 0, lang);
	...
	public void getParentMutId(Map mutation) {
		...
		if (parentVariant.isEmpty() && (variant.indexOf("fs") > -1 || variant.indexOf("*") > -1 || variant.indexOf("+") > -1 || variant.indexOf("-") > -1)) {
	...
	public Map<String, String> matchNKB_readonly(String gene, String variant, String ori_variant,Integer diseaseId, Integer lang) {
		...
		List<Integer> parentdiseaseIdList = new ArrayList<>();
		getDiseaseList(diseaseId, diseaseIdList, parentdiseaseIdList);
		reportCrService.matchNKBVarDrug(a, gene, variant, ori_variant, diseaseId, diseaseIdList, parentdiseaseIdList, lang);
		return a;
	}
}
