...
public class ImageMagickUtils {
     ...
     * 水印 
     */  
    private static String watermarkImagePath = "";  
    ...
    static {  
        try {  
          ...
          /*  watermarkImagePath = SystemConfig.getWatermarkImagePath();  
            watermarkImage = ImageIO.read(new File(SystemConfig.getWatermarkImagePath()));  */
        } catch (Exception e) {  
            e.printStackTrace();  
        }  
    }  
    ...
    private static ImageCommand getImageCommand(CommandType command) {  
        ...
        if (cmd != null && System.getProperty("os.name").toLowerCase().indexOf("windows") != -1) {  
            cmd.setSearchPath(USE_GRAPHICS_MAGICK_PATH ? GRAPHICS_MAGICK_PATH : "/usr/bin");  
        }  
        return cmd;  
    }  
    ...
    public static void addTextWatermark(String srcImagePath, String destImagePath, String content)  
        ...
        // 文字信息  
        op.pointsize(18).fill("#BCBFC8").draw("text 80,10 " + content);  
        // 原图  
        op.addImage(srcImagePath);  
        // 目标  
