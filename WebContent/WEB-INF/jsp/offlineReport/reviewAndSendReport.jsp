	     ...
	     <tr>
	    	<td>
		      <div class="form-group">
		        ...
		        <div class="field">
		          <input type="text" id="report_filenameone" name="report_filenameone" readonly="readonly" class="input w50" style="width:350px;float: left;" value="${offlineReport.report_filenameone }"  />
		          <div class="tips"></div>
		     ...
		     <td>
				<div class="form-group">
					...
					<button id="updateReportFileOne"  style="width: 150px;" class="button bg-main icon-file-o" > 更换报告文件</button>
				</div> 
	     ...
	     <tr>
	    	<td>
		      <div class="form-group">
		        ...
		        <div class="field">
		          <input type="text" id="report_filenametwo" name="report_filenametwo" readonly="readonly" class="input w50" style="width:350px;float: left;" value="${offlineReport.report_filenametwo }"  />
		          <div class="tips"></div>
		          <input type="hidden" id="report_file_path" name="report_file_path" value="${offlineReport.report_file_path }"  />
		        </div>
		      </div>
		     ...
		     <td>
				<div class="form-group">
					...
					<button id="updateReportFileTwo"  style="width: 150px;" class="button bg-main icon-file-o" > 更换报告文件</button>
				</div> 
				<input type="hidden" name="report_id"  value="${offlineReportIframeBean.report_id}" >
			</td> 
	     </tr>
			...
			<table style="width:100%">
				<tr>
				     ...
				     <td>
				        ...
				        <div class="field">
				        	<button id="sendEmail" style="width: 150px;margin-right: 75px;" disabled="disabled" class="button bg-main icon-send" > 发送邮件</button>
					        <script type="text/javascript">
					        $(function(){
				        		...
				        		$("#review").click(function(){
				        			$.myConfirm({title:'报告审核确认',message:'确认审核通过后将无法更换报告文件！',callback:function(){
			        					$.post("${pageContext.request.contextPath}/offlineReport/editStatus",{"report_id":"${offlineReportIframeBean.report_id}","status":"审核通过"},
				        					function(data){
				        						$("#updateReportFileOne").prop("disabled","disabled");
							        			$("#updateReportFileTwo").prop("disabled","disabled");
							        			$("#review").prop("disabled","disabled");
				        		...
				        		$("#reviewfalse").click(function(){
				        			$.myConfirm({title:'报告审核确认',message:'确认审核未通过后将更改报告状态！',callback:function(){
				        				$.post("${pageContext.request.contextPath}/offlineReport/editStatus",{"report_id":"${offlineReportIframeBean.report_id}","status":"审核未通过"},
					        					function(data){
